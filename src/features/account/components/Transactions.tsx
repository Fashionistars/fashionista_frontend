...
// Collapsible is used for the FAQ accordion below
  ...
  const faqList = faq.map((question, index) => {
    return (
      <div key={index} className="w-1/2">
        <div className="flex flex-col gap-1 border-b border-[#d9d9d9] py-2">
          <button
            className="flex items-center gap-2 text-left font-satoshi text-lg"
            onClick={() => setOpenFaqIndex(openFaqIndex === index ? null : index)}
          >
            <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg" className={`transition duration-200 ${openFaqIndex === index ? 'rotate-180' : 'rotate-0'}`}>
              <path d="M13.5 6.75004C13.5 6.75004 10.1858 11.25 9 11.25C7.8141 11.25 4.5 6.75 4.5 6.75" stroke="black" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
            {question.title}
          </button>
          {openFaqIndex === index && (
            <p className="text-[13px] text-[#222] capitalize">{question.text}</p>
          )}
        </div>
      </div>
    );
