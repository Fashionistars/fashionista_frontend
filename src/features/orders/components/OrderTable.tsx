...
const OrderTable = () => {
  ...
  const orderItems = filteredOrders.map((item) => (
    ...
    >
      ...
      <td className=" text-black py-2">{item.order_status}</td>
      <td className=" text-black py-2">{item.items}</td>
    </tr>
  ));
      ...
      <div className="px-5 md:px-10 space-y-10">
        ...
        <div className="flex items-center w-[60%] h-10 md:w-1/3 md:h-[60px] bg-white border border-[#d9d9d9] px-4 gap-6 rounded-[5px]">
          <span className="block transition-all peer-focus:hidden">
            <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M13.125 13.125L16.5 16.5" stroke="#282828" strokeLinecap="round" strokeLinejoin="round" />
              <path d="M15 8.25C15 4.52208 11.978 1.5 8.25 1.5C4.52208 1.5 1.5 4.52208 1.5 8.25C1.5 11.978 4.52208 15 8.25 15C11.978 15 15 11.978 15 8.25Z" stroke="#282828" strokeLinejoin="round" />
            </svg>
          </span>
          <input
            type="search"
        ...
        <div className="space-y-6">
          <nav className="w-full lg:w-[55%] flex justify-between items-center font-satoshi font-medium">
            {[
              { label: `All(${orders.length})`, status: "" },
              { label: "Pending", status: "pending" },
              { label: "Ready to deliver", status: "ready-to-deliver" },
